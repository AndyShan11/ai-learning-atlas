...
function renderUpdates() {
  $("#content").innerHTML =
    `<h2 class="view-title">更新知识，而不追逐名词。</h2><p class="view-lead">基于 AMAI 2022 路线图的重构思路，重新编写内容与界面。核查日期 ${reviewed}；不是“所有 2026 技术”的穷尽目录。</p><div class="update-block"><h3>2.1 · 覆盖审计与扩充</h3><p>本次新增 88 个节点、64 条跨树关系。对照 roadmap.sh、scikit-learn 与 Hugging Face 等资料，补齐分析交付、可信验证、专门任务、现代生成与计算效率。</p><ul><li>分析岗：BI、星型模型、实验检查、效应大小与多重检验。</li><li>Kaggle：Pipeline、OOF、嵌套验证、不平衡处理、消融、评价指标与榜单纪律。</li><li>AI：MoE、Mamba、时间序列基础模型、VAE / GAN、混合检索、RLVR、Agent 工具链与推理缓存。</li></ul><p>新加入图中不等于 2026 年发明。前沿家族链接到维护方，具体版本、许可和硬件限制按使用时核查。</p><p><a href="./COVERAGE_AUDIT.md">查看本次查漏记录与资料对照 ↗</a></p><p>${nodes.length} 个节点、${nodes.length - 1} 条层级连线、${relations.length} 条带类型的跨树关系。补齐各模块的多级子树；同图提供星系和树形布局、缩放拖动、祖先高亮、关系线、折叠与完整 SVG 导出。</p><h3>知识组织原则</h3><ul><li>从职业工具清单改为 8 个知识模块，分开学习方式、模型、验证和训练技巧。</li><li>每个节点增加定义、适用条件、使用边界、例子和关联知识。</li><li>新增数据分析 / Kaggle / 生成式 AI 三条学习主线，以及条件选择器与组合方案。</li><li>经典不等于淘汰。“按需”表示对当前目标的优先级，不表示业界弃用。</li></ul></div><div class="update-block"><h3>内容取舍记录</h3><table class="updates-table"><thead><tr><th>内容</th><th>处理与原因</th></tr></thead><tbody><tr><td>LoRA / QLoRA、DPO、GRPO、TabPFN</td><td>补充到适配、后训练、表格模型的位置，写明资源、反馈与规则边界。不是所有任务的默认选择。</td></tr><tr><td>RAG、嵌入、多模态与生成评估</td><td>补充现代应用链路。“新增到本图”不代表发明于 2022 年后；RAG 原论文发表于 2020 年。</td></tr><tr><td>线性模型、树模型、TF-IDF、统计与实验</td><td>保留。仍能提供低成本基线或关键分析能力，不因年代较早而移除。</td></tr><tr><td>RNN / LSTM、SVM</td><td>保留为按需分支。说明适配场景，不放在所有任务的默认主线上。</td></tr><tr><td>大数据平台与分布式技术清单</td><td>缩到工程模块，等规模需求明确再学。对当前数据分析与 Kaggle 目标不必优先。</td></tr><tr><td>余弦退火、温度缩放、阈值、融合</td><td>按训练 / 后处理归位，并区分优化、校准与决策，不再散落在模型树上。</td></tr></tbody></table></div><div class="update-block"><h3>来源与维护</h3><p>节点正文是简明教学归纳，方法适用性是条件建议。重点核查新方法及容易混淆的概念；参考链接指向官方文档或原始论文。滚动文档会变化，具体版本限制以原文为准。</p><ul class="source-links">${["roadmap2026", "bi", "imbalance", "cookbook", "chronos", "mamba", "cache", "selection", "rl", "peft", "dpo", "grpo", "tabpfn", "temperature", "cosine", "rag", "tuning"].map((id) => `<li><a href="${sources[id][1]}" target="_blank" rel="noopener">${esc(sources[id][0])} ↗</a></li>`).join("")}<li><a href="https://github.com/AMAI-GmbH/AI-Expert-Roadmap" target="_blank" rel="noopener">AMAI 原版路线图 · 灵感来源 ↗</a></li><li><a href="./KNOWLEDGE.md">完整文字版与参考链接 ↗</a></li></ul></div>`;
}
function toast(text) {
