...
- **220 个节点、219 条层级连线、55 条跨树关系**：覆盖 8 个模块，展开到最多 6 层。节点包含模块、子类与具体方法。
...
`data.js` 保留第一版笔记与学习方案；`universe-data.js` 定义增补笔记、完整树、带类型关系。`universe.js` 负责布局和画布交互，`app.js` 负责页面与详情。每个节点保留稳定 `id`，旧分享链接继续可用。
