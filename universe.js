...
export function createUniverse(container, { onSelect }) {
  ...
  const defs = `<defs>${[...domains, { id: "core", color: "#efd59a" }].map((d) => `<radialGradient id="planet-${d.id}" cx="30%" cy="25%" r="77%"><stop offset="0" stop-color="#ffffff"/><stop offset=".18" stop-color="${d.color}"/><stop offset=".57" stop-color="${d.color}" stop-opacity=".55"/><stop offset="1" stop-color="#080c19"/></radialGradient>`).join("")}<filter id="soft-glow" x="-150%" y="-150%" width="400%" height="400%"><feGaussianBlur stdDeviation="14"/></filter><marker id="arrow-applies" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0 L10 5 L0 10" fill="#83b9ff"/></marker><marker id="arrow-requires" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0 L10 5 L0 10" fill="#e1bf79"/></marker></defs>`;
  container.innerHTML = `<svg class="universe-svg" tabindex="0" role="group" aria-label="完整知识宇宙。滚轮缩放、拖动平移。Tab 选择节点，Enter 查看，双击聚焦。" xmlns="${NS}">${defs}<g class="universe-world"><g class="orbits" aria-hidden="true"></g><g class="tree-edges" aria-hidden="true"></g><g class="semantic-edges" aria-hidden="true"></g><g class="planet-nodes"></g></g></svg><div class="canvas-hint">拖动探索 · 滚轮缩放 · 双击聚焦子树</div><div class="canvas-scale" aria-live="polite"></div><div class="mini-map" aria-label="全图定位"><svg viewBox="-2100 -2100 4200 4200" aria-hidden="true"></svg><span>全图定位</span></div>`;
    ...
    orbits.innerHTML =
      mode === "orbit"
        ? [800, 1160, 1520, 1880, 2240]
            .map(
              (r) =>
                `<circle r="${r}" fill="none" stroke="#93a7d0" stroke-opacity=".055" stroke-width="1"/>`,
            )
            .join("")
        : "";
      ...
      .map((n) => {
        ...
        const labelX = outward
          ...
          : main
            ...
            : flip
              ? -r - 12
              : r + 12;
        const labelY = main && !outward ? r + 26 : 5;
        return `<g class="planet-node depth-${n.depth}" data-id="${n.id}" data-depth="${n.depth}" tabindex="0" role="button" aria-label="${xml(n.title)}${n.children.length ? "，" + n.children.length + " 个直接子节点" : ""}" aria-pressed="false" transform="translate(${p.x} ${p.y})"><title>${xml(
          ancestors(n.id)
            .map((n) => n.title)
            .join(" → "),
        )}</title>${main ? `<circle class="planet-halo" r="${r * 1.7}" fill="${color(n)}" opacity=".14" filter="url(#soft-glow)"/>` : ""}<circle class="selection-orbit" r="${r + 8}" fill="none" stroke="${color(n)}" stroke-width="1.2" opacity="0" vector-effect="non-scaling-stroke"/>${main ? `<ellipse rx="${r * 1.5}" ry="${r * 0.34}" transform="rotate(-28)" fill="none" stroke="${color(n)}" stroke-opacity=".36" stroke-width="1.3"/>` : ""}<circle class="planet-body" r="${r}" fill="${main ? "url(#planet-" + (n.domain || "core") + ")" : color(n)}" stroke="${color(n)}" stroke-width=".6"/><circle class="node-hit" r="${Math.max(17, r)}" fill="transparent"/><g class="label-group" transform="rotate(${angle})"><text class="planet-label" x="${labelX}" y="${labelY}" text-anchor="${outward ? (mainLeft ? "end" : "start") : main ? "middle" : flip ? "end" : "start"}" fill="${main ? "#e8ecf7" : "#bbc8e0"}" font-size="${main ? 23 : 14}" paint-order="stroke" stroke="#070d1d" stroke-width="4" stroke-linejoin="round">${xml(n.title)}</text></g>${collapsed.has(n.id) ? `<text class="collapsed-count" y="${-r - 12}" text-anchor="middle" fill="#f7d297" font-size="15">+${descendants(n.id).length - 1}</text>` : ""}</g>`;
      })
      .join("");
    mini.innerHTML =
      hierarchy
  ...
  function highlight() {
    ...
    nodeLayer.querySelectorAll(".planet-node").forEach((el) => {
      ...
      el.classList.toggle("reveal-label", subtree.size <= 20 ? subtree.has(id) : nodeMap.get(id).parent === selected);
      el.classList.toggle(
        "off-focus",
        Boolean(selected) &&
          !chain.has(id) &&
  ...
  function transform() {
    ...
    nodeLayer.querySelectorAll(".planet-node").forEach((el) => {
      ...
      const important =
        ...
        el.classList.contains("neighbor") || el.classList.contains("reveal-label");
      let shown =
        allLabels ||
        n.depth <= 1 ||
        important ||
